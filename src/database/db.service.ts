...
export class DatabaseService {
    ...
    private async getRows(values: string = "*", table: string, where: string, params?: any[], additional_query: string = "") {
        const res = await this.doSql(`SELECT ${values} FROM ${table} WHERE ${where} ${additional_query}`, params)
        return res;
    }

    private async getSingleRow(values: string = "*", table: string, where: string, params?: any[], additional_query: string = "") {
        const [res] = await this.getRows(values, table, where, params, additional_query);
        return res;
    }

    private async insertRow(table: string, columns: string, values: string, params?: any[], additional_query: string = "") {
        const [response] = await this.doSql(`INSERT INTO ${table} (${columns}) VALUES (${values}) ${additional_query}`, params)
        if (response) return response;
    }

    private async deleteRow(table: string, where: string, params?: any[], additional_query: string = "") {
        await this.doSql(`DELETE FROM ${table} WHERE ${where} ${additional_query}`, params)
    }

    private async updateRow(table: string, values: string, params?: any[], additional_query: string = "") {
        await this.doSql(`UPDATE ${table} VALUES SET ${values} ${additional_query}`, params)
    }
    // </core>
    ...
    async createUser(permissions: Permissions) {
        ...
        const res = await this.insertRow("Users", "permissions", "$1", [convertedPermissions], "RETURNING user_id")
    ...
    async getUserIntegrationsDiscord(discord_id: bigint, values: string = "*") {
        const integrations = await this.getSingleRow(values, "UsersIntegrations", "discord_id = $1", [discord_id])
        if (!integrations) return;
        return integrations as UserIntegrations;
    }

    async setUserIntegration(user_id: bigint, key: string, value: string | number | bigint) {
        if (await this.getUserIntegrations(user_id)) {
            await this.updateRow("UsersIntegrations", `${key} = $1`, [value])
        } else {
            await this.insertRow("UsersIntegrations", `user_id, ${key}`, "$1, $2", [user_id, value])
        }
    }
    // </auth>
}
